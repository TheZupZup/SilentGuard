...
redirect the connection-state cache, the mitigation state/audit files,
and the event-history store to per-test temp files via autouse
fixtures so any handler or monitor code path that touches them during
a test stays isolated.
"""
...
from silentguard import connection_state, events, mitigation
...
def _isolate_events_file(tmp_path, monkeypatch):
    events_file = tmp_path / "silentguard_events.json"
